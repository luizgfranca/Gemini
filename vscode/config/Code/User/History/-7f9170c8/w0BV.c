...
int main() {
    ...
    if(meta == NULL) {
        metadata* base = create_metadata_def(1);
        if( save_metadata( base ) != SUCCESS ) {
            printf("failed to create metadata file\n");
            exit(1);
        }
    ...
    event_record_node* first_record = read_new_events( meta );
    if(first_record == NULL) {
        printf("No events registered\n");
    }

    print_event_record_list(first_record);
