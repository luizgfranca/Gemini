#include "settings.h"

#include "system/file-utils.h"
#include "event-file.h"
#include "pending-job-queue.h"
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>

int main() {
    metadata* meta = read_metadata();
    
    if(meta == NULL) {
        if( save_metadata(create_metadata_def(0) ) != SUCCESS ) {
            printf("failed to create metadata file\n");
            exit(1);
        }

        meta = read_metadata();
        if(meta == NULL) {
            printf("failed metadata read after write\n");
            exit(1);
        }
    }

    printf("metadata: offset=%lu\n", meta->event_file_offset);



    event_record_ref* first_record_ref;
    event_record_ref* new_records_first;

    while(1) {
        new_records_first = read_new_events( meta );
        if(*new_records_first == NULL) {
            printf("No new events registered\n");
        } else if(first_record_ref == NULL)  {
            first_record_ref = new_records_first;
        } else {
            pending_job_queue__concat(first_record_ref, new_records_first);
        }

        print_event_record_list(first_record_ref);
        sleep(POLL_INTERVAL_SECONDS);
    }

    return 0;
}