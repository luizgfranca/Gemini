...
void MainWindow::load_grid_data() {
    ...
    auto scroller_position = m_service_grid_scroller.get_cursor();

    // double x, y;
    // m_service_grid_scroller.translate_coordinates(m_treeview, 0, 0, x, y);
    // std::cout << "x, y" << x << ", " << y << "\n";
    std::cout << "*****\n";

    auto grid_allocation = m_treeview.get_allocation();

    m_tree_store_mutex.lock();
    m_tree_store->clear();
    std::cout << "cleared tree store\n";

    // auto scroller_cursor = m_service_grid_scroller.get_vscrollbar()->get_adjustment();
    ...
    if(scroller_position) {
        m_service_grid_scroller.set_cursor(scroller_position);
    }
