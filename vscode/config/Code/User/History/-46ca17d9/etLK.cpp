...
MainWindow::MainWindow(SismApplication *application) {
    ...
    m_refresh_button.signal_clicked().connect(sigc::mem_fun(*this, &MainWindow::on_restart_service_click));
...
void MainWindow::stop_service() {
    auto item = get_service_from_currently_selected_row();
    ...
    auto item = (*m_application->system_services_controller().get_services())[std::stoi(selected.at(0).to_string())];
