...
void SystemServicesController::refresh() {
    auto original_services_list = provider::systemd::Unit::filter_by_type(m_manager.get_services_list(), "service");
    provider::systemd::Unit::sort_by_name_inplace(original_services_list)

    return original_services_list;
}
