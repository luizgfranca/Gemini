...
std::unique_ptr<std::vector<Unit>> Unit::from_dbus_list_units_and_list_files_response(
    ...
    auto raw_units = std::make_shared<std::vector<Unit>>(
        Unit::from_dbus_list_units_response(list_units_response)
    );
    ...
    auto final_unit_list = std::make_unique<std::vector<Unit>>();
    // considering that
    ...
    for(auto& file : *unit_files) {
        if(file.name == raw_units->at(unit_list_position).name) {
