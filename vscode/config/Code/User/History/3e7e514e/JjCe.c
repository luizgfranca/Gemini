...
void _event_record_list__foreach(event_record_ref* first, void (*fn)(event_record_node*)) {
    assert(first != NULL);
    event_record_ref* it = first;
    while((*it) != NULL) {
        fn(*it);
        assert((*it) != NULL);
        EVENT_RECORD_LIST__NEXT((*it));
    }
}
...
void print_event_record_list(event_record_ref* _ref) {
    _event_record_list__foreach(_ref, _event_record__print);
}
...
event_record_ref* read_new_events(metadata* _metadata) {
    ...
    event_record_node* list_first_node = NULL;
    event_record_node* list_curr_node = NULL;
        ...
        switch (c) {
            case EVENT_SEPARATOR:
                if(list_curr_node == NULL) {
                    list_curr_node = _create_list_with_first_element(buffer, curr_offset);
                    ...
                    EVENT_RECORD_LIST__NEXT(list_curr_node);
                    assert(list_curr_node != NULL);
                }
    ...
    event_record_ref* result = &list_first_node;
