...
void _event_record_list__foreach(event_record_ref* first, void (*fn)(event_record_ref*)) {
    printf("_event_record_list__foreach");
    event_record_ref* it = first;
    while(it != NULL) {
        fn(it);
        assert(it != NULL);
        if((*it)->next != NULL)
...
void _append_record(event_record_ref* _curr, char* _buff, u_int16_t _len) {
    event_record_node* new = _create_new_event_record(_buff, _len);
    assert(new != NULL);

    if(_curr == NULL) {
        _curr = malloc(sizeof(event_record_node*));
        *_curr = new;
        return;
    }
...
void _event_record__print(event_record_ref* _node) {
    // printf("event_record: %s", _node->content);
}
...
event_record_ref* read_new_events(metadata* _metadata) {
    ...
    static event_record_ref* list_first_node = NULL;
    static event_record_ref* list_curr_node = NULL;
    ...
    while((c = (char) fgetc(fp)) > 0) {
        ...
        switch (c) {
            case EVENT_SEPARATOR:
                ...
                if(list_first_node == NULL) {
                    list_first_node = list_curr_node;
                }

                

                if(_EVENT_RECORD_LIST__HAS_NEXT(list_curr_node)) {
